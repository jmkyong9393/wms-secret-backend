...
### 3.1.3 물리-디지털 싱크(Sync) 문제 극복: Two-Step 공정 분리
소프트웨어 상의 '비동기 30초 대기'와 현실 물류 현장의 엇박자를 고가의 하드웨어(Auto-Sorter) 없이 해결하기 위해 **물리적 버퍼(Pending Bin)** 개념을 도입했습니다.
1. **고속 촬영 공정 (Input):** 작업자는 도서 바코드를 스캔하고 훼손 부위를 촬영한 뒤, 즉시 책을 '판정 대기함(Pending Bin)'에 넣습니다. (작업자 대기 시간 0초)
2. **확정 분류 공정 (Sorting):** 수 분 뒤 대기함에 쌓인 책들을 꺼내 바코드를 다시 스캔하면, 백그라운드 AI 판독이 완료된 상태이므로 화면에 0.1초 만에 즉각적(Sync)으로 "A급", "폐기" 등의 분류 지시가 출력됩니다.
