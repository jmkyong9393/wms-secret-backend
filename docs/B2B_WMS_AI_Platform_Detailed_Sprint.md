...
### 🏃 Sprint 1 (W1): 기반 공사 및 R&R 로드밸런싱 적용
* **[1주차] 환경 세팅 및 아키텍처 설계**
  * `[Tech PM]`: 전체 데이터 그리드 셋업 및 GitHub Actions CI/CD 인프라 구축.
  * `[BE-3]`: 도메인 분석, 복합키 기반 PostgreSQL DDL 스키마 작성 및 FastAPI/SQLModel 진입점 구축.
  * `[FE-1]`: 타사 환불 정책 리서치 기반 RAG 검색용 지식베이스(YAML) 취합 완료 및 Next.js PWA 카메라 연동 초안 구축.
  * `[AI Lead]`: LangGraph 프레임워크 4-Agent 상태 머신 구조 뼈대 세팅 및 UBCI 상태 평가지수 파라미터화.
  * `[BE-1]`: ChromaDB 파이프라인 세팅 및 지식베이스 임베딩 전처리 스크립트 구축, 테스트 원시 데이터 100장 수집.
  * `[BE-2]`: FastAPI 프로젝트 세팅, Redis/Celery 비동기 큐 뼈대 및 SSE 실시간 푸시 엔드포인트 초안 작성.
  * `[FE-2]`: Next.js 기반 관리자 대시보드 기획 및 Jotai 실시간 검수 큐 상태관리 뼈대 설계.
...
* **[4주차] 에이전트 로그 대시보드 & 예외 처리**
  ...
  * `[FE-1]`: 실제 물류센터 환경 E2E 통합 테스트 지원 및 모바일 코드 인수인계 완료. (이후 태스크 없음 - 조기 Code Freeze)
