...
위 전제가 실측으로 반증되어 게이트를 비활성했다 (History §14).
  ...
  1) "학습 도메인 = 책이 프레임을 가득 채운 근접 촬영"이 사실이 아니다. Roboflow 학습본 811장을 표본 조사한 결과 사람이 찍은 사진 그대로이며 책이 프레임의 60~80%를 차지하고 책상 배경이 남아 있다. ROI 크롭(패딩 4%)은 책이 92%를 채우므로 학습 도메인에 맞추는 것이 아니라 오히려 벗어나게 만든다.
  2) COCO book 게이트는 팀 실촬영 71장 중 26장(37%) 에서만 책을 찾았다. 실패한 45장은 전부 "미검출"이고 ROI 과소로 걸러진 것은 0장이다 - 크기 문제가 아니라 COCO가 이 사진들을 책으로 보지 않는다(책배·속지 촬영본, 프레임을 꽉 채운 표지컷).
  3) 크롭 유무를 동일 이미지에 직접 대조한 결과 적중 차이는 ±1~2개이고 방향도 일정하지 않다. 크롭이 이득이라는 증거가 없다.

  스케일 불일치(팀 실촬영 결함이 프레임 대비 2.35% vs Roboflow 3.3~5.6%)는 사후 크롭이 아니라 촬영 규격(책상 평면, 책이 화면을 채우게)으로 해소한다. 코드는 지우지 않고 USE_ROI_GATE 플래그로 남겨 A/B 가능하게 둔다.
====================================================================
"""
...
# ====================================================================
# Phase 1: Stage 0 ROI 게이트 비활성화 + 추론 해상도 640 정렬
# ====================================================================
# 결함 3모델의 학습셋(Roboflow)은 사람이 찍은 사진 그대로이며, 책이 프레임의 60~80%를 차지하고 책상 배경이 남아 있다. 반면 Stage 0 ROI 크롭은 패딩 4%라 책이 92%를 채워, 크롭이 학습 도메인에 맞추는 것이 아니라 오히려 벗어나게 만든다.
# 실측 근거 (History §14.1 / §14.3):
#   - COCO book 게이트는 팀 실촬영 71장 중 26장(37%)에서만 책을 찾았고, 실패 45장은 전부 "미검출"이었다 (ROI 과소 0장). 크기 문제가 아니라 COCO가 우리 사진을 책으로 보지 않는다.
#   - 크롭 유무를 직접 대조한 결과 적중 차이는 ±1~2개이고 방향도 일정하지 않다. 크롭이 이득이라는 증거가 없다.
# 코드는 지우지 않고 플래그로 남긴다 - 새 촬영 규격으로 테스트하면서 A/B가 필요하다.
USE_ROI_GATE = False

# 결함 3모델의 추론 해상도. Model 1(v6_recall)의 학습 해상도가 640이므로 일치시킨다.
# 동일 가중치로 해상도만 올려 측정하면 Roboflow mAP50이 0.530(@640) -> 0.464(@1024)로 12~19% 하락한다. 원본이 640인 데이터를 키우는 것은 정보를 더하지 않고 보간 흐림만 만든다.
DEFECT_IMGSZ = 640

# ====================================================================
# [Track 2·3] VLM 크롭 실측 데이터 적재
# ====================================================================
# VLM이 속지/책배를 제대로 잘랐는지는 눈으로 확인해야만 알 수 있다.
# 좌표만 로그로 남기면 "박스가 나왔다"는 사실만 알 뿐 그 박스가 맞는지는 판단할 수 없다.
...
# ====================================================================
# 속지 크롭 좌표를 VLM에게 받아 봤으나 정확도가 낮았다.
# 실측: GPT-4o가 반환한 박스는 {xmin:50, ymin:100, xmax:950, ymax:900} 처럼 전부 반올림된 숫자로, 실제 지면 경계를 찾은 것이 아니라 "가운데 90%x80%" 정도를 추측한 값이었다.
# YOLO-World는 텍스트 프롬프트로 대상을 지정하는 검출기라 COCO 고정 클래스의 한계가 없다.
# 실측 비교 (팀 실촬영 6장, 프롬프트 "book", imgsz=640):
#   yolov8x-worldv2 : conf 0.48~0.95, 6장 전부 책 영역만 정확히 포착
# 역할 분리: VLM은 "몇 번 사진이 속지인가"(의미 판단), YOLO-World는 "정확히 어디인가"(공간 판단)를 맡는다. 각 도구의 강점에 맞춘 배치다.
...
# ── 손 제거를 시도했다가 철회한 이력 ──────────────────────────
#
# 크롭에 손이 남는 것을 없애려고 COCO person 박스로 책 박스의 위/아래 가장자리를 잘라내는 로직을 넣었다가 철회했다.
#
# 실측(팀 실촬영 6장): 5장에서 손이 제거됐으나, neg_006에서 손이 책 모서리를 감싸 쥔 구도라 손을 피하는 과정에서 표지 하단까지 잘려나갔다.
# 이 경로의 목적은 찢어짐·마모 탐지이므로 책 영역이 소실되면 결함 자체를 놓친다. 손을 지우자고 결함을 놓치는 것은
# 본말전도다.
#
# 근본적으로 축정렬 박스로는 "책은 보존하면서 손만 제거"가 성립하지 않는다.
# 손이 책을 쥐고 있으면 손 영역을 빼는 순간 그 아래 책 영역도 함께 날아간다.
# YOLO-World가 반환하는 책 박스는 이미 책 경계까지만 잡으므로 그 바깥의 손·책상은 자동으로 제외되며, 안쪽으로 더 깎는 것은 책을 훼손하는 것 이상의 의미가 없다.
#
# 손 픽셀을 실제로 지우려면 박스가 아니라 세그멘테이션 마스크가 필요하다.
...
def detect_page_region(
    ...
    찾지 못하면 None - 호출부는 VLM이 준 좌표로 폴백한다.
    """
...
def ensure_model_weights() -> None:
    """
    로컬에 없는 YOLO 가중치 파일(.pt)을 S3(s3://{AWS_S3_BUCKET}/models/)에서 내려받아 정확히 위 경로 상수 자리에 저장한다.
    Docker 이미지/git 저장소에 150MB짜리 가중치를 더 이상 baked-in 하지 않기 위한 최초 1회(워커 프로세스당) 다운로드 로직.

    자격증명이 없으면 조용히 폴백하지 않고 명시적으로 예외를 던진다 - 모델이 없으면 검수 자체가 불가능하므로 목업으로 얼버무릴 수 없는 종류의 실패다.
...
class WBFBookDefectDetector:
    ...
    def detect_defects_wbf(
        ...
        """
        단일 이미지에 대해 [Stage 0 책 ROI 크롭] -> [3대 모델 WBF 앙상블] 결함 추론을 실행하고 융합된 BBox 리스트를 반환합니다.
        반환 bbox는 ROI 적용 여부와 무관하게 항상 원본 이미지 기준 0~1000 상대좌표다.
        ...
        for box, score, label in zip(f_boxes, f_scores, f_labels):
            # 추론 영역(ROI 또는 풀프레임)의 대부분을 덮는 거대 박스는 결함이 아니라 배경/도메인 오탐 신호이므로 폐기한다. (실물 결함이 책의 80%를 덮는 경우는 없다.)
            if (box[2] - box[0]) * (box[3] - box[1]) > max_box_area_ratio:
                continue

            # 융합 후 신뢰도 하한. 모델별 conf는 낮게 두어 미탐을 막되(넓게 던지고), 밖으로 내보내는 후보는 이 선에서 끊는다. 후보는 VLM 판독 힌트이자 HITL 화면의 증거로 쓰이므로, 낮은 점수 잡음이 섞이면 판독을 흐리고 검수자가 엉뚱한 좌표를 확인하느라 시간을 쓴다.
    ...
    def detect_doodle_only(
        ...
        Track 1(앞면·뒷면·책등)에서는 표지에 쓴 이름·낙서를 잡기 위해 기존 3모델 앙상블을 그대로 쓴다. 즉 같은 모델을 두 진입점으로 사용한다.

        Args:
            image_bgr: 크롭된 BGR 이미지 (파일 경로가 아니라 배열을 받는다 - VLM이 지정한 영역을 호출부에서 이미 잘라낸 상태이기 때문).
            debug_tag: 지정하면 크롭본과 탐지 결과를 로컬에 남긴다 (SAVE_VLM_CROPS 참조). VLM이 속지를 제대로 잘랐는지는 눈으로 확인해야만 알 수 있으므로, 실측 데이터를 쌓아 두고 나중에 검증한다.
            debug_meta: 사이드카 JSON에 함께 기록할 정보 (원본 파일명, VLM이 준 박스, 판별된 사진 종류 등).
        Returns:
            `detect_defects_wbf()`와 **동일한 형식**. bbox는 입력 이미지(=크롭본) 기준 0~1000 정규화 좌표이므로, 원본 좌표가 필요하면 호출부에서 역변환해야 한다.
        """
