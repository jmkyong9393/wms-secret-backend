...
def run_policy_node(state):
    ...
    [공식 4대 등급 및 점수 기준표]
    - MINT (S등급) : 90점 ~ 100점 (결함 없음 / 미세 상해)
    - GOOD (A등급) : 75점 ~ 89점 (양호한 사용감)
    - NORMAL (B등급) : 60점 ~ 74점 (보통 사용감 및 일반 결함)
    - REJECT (반려) : 0점 ~ 59점 (심각한 훼손, 곰팡이/물젖음, 재판매 불가)
    ...
    {{"**[중요] 이 도서는 수험서/자격증/문제집 카테고리입니다. 문제집 특성상 필기/낙서/밑줄은 필수적이므로, 필기/낙서/밑줄에 의한 감점은 최대 -15점으로 한정(Cap)하여 그만큼만 감점 처리하세요. 단, 찢어짐이나 물젖음 등 다른 물리적 '심각한 훼손'이 동반된 경우에 한해 추가 감점하여 REJECT(0~59점)로 분류할 수 있습니다. 계산된 최종 점수에 맞는 등급과 규격화된 사유를 작성하세요.**" if is_workbook else ""}}
