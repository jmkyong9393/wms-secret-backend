...
class PolicyClauseCitation(BaseModel):
    ...
    chunk_id: str = Field(default="", description="지식베이스 청크 ID (내부 감사용, 고객 노출 금지)")
...
class ReturnPolicyVerdict(BaseModel):
    ...
    shipping_fee_bearer: Optional[Literal["CUSTOMER", "SELLER", "CARRIER"]] = Field(
        default=None, description="반품 배송비 부담 주체. 근거가 없으면 null",
    )
    ...
    cited_clauses: List[PolicyClauseCitation] = Field(
        default_factory=list, description="위 판단의 근거 조항. 비어 있으면 판단은 무효다",
    )
    ...
    rationale: str = Field(
        default="", description="판단 근거 2~4문장 한국어. 인용한 조항명을 문장 안에 밝힐 것",
    )
