...
def build_yolo_hint(yolo_candidates: List[Dict[str, Any]]) -> str:
    ...
    return (
        ...
        # 좌표 접지 규칙 - VLM은 정규화 좌표를 스스로 만들지 못한다(실측: 세 컷에 같은
        # 좌상단 박스를 지어내 비접지 게이트에 걸림). 후보 좌표는 픽셀 실측이므로 그대로 쓴다.
        "후보 위치에서 결함을 확인했으면 **bbox는 그 후보의 좌표를 그대로 사용**하라 - 그 좌표는 탐지 모델의 픽셀 실측이다.\n"
        "단 **유형(type)은 네가 다시 판정**하라 - 예: 모서리 접힘이 TEAR로 제보됐어도 실제가 접힘이면 type만 DMG_EXT_CRUSH로 바꾸고 bbox는 후보 좌표를 유지한다.\n"
