...
ReasonCode = Literal[
    ...
    "OK",
    "REJECT",
    "MAX_RETRIES_AMBIGUOUS_HITL",
    "BOUNDARY_AMBIGUOUS_HITL",
    "HUMAN_REQUIRED",
...
class WMSInspectionState(TypedDict):
    ...
    # 이 State에 선언되지 않아 book_title과 같은 이유로 버려지고 있었다.
    ...
    yolo_candidates: Optional[
        list
    ]  # WBF 앙상블 사전탐지 후보 (image_index/type/bbox/confidence)
    ...
    special_notes: Optional[
        str
    ]  # UBCI 감점과 무관한 정성적 관찰 (도서관 도장, 부록 누락 등)
    ...
    human_feedback: Optional[
        str
    ]  # 관리자 입력값 ("approve", "reject", "recalculate" 등)
