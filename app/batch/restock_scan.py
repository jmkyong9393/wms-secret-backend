...
배치 파드는 Celery 워커 풀에 스캔 태스크를 큐잉만 하고 즉시 종료하므로 LLM 비용이 큰 실제 스캔은 acks_late 보장이 있는 상시 워커에서 수행된다.
브로커(Redis) 장애 시에는 배치 파드 안에서 스캔을 직접 실행(fail-open)해 "그날의 스캔이 통째로 증발"하는 상황을 막는다.
"""
import logging
