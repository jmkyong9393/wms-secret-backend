...
class User(SQLModel, table=True):
    ...
    status: UserStatusEnum = Field(
        ...
        sa_column=Column(
            SQLEnum(UserStatusEnum), nullable=False, default=UserStatusEnum.ACTIVE
        ),
    )
...
class Book(SQLModel, table=True):
    ...
    standard_size: Optional[str] = Field(
        default=None, max_length=50
    )  # BoxStandard Enum Name
    ...
    calc_source: Optional[str] = Field(
        default="ALADIN_REAL_SPEC", max_length=50
    )  # 물리 규격 수집 출처
...
class InventoryUsedItem(SQLModel, table=True):
    ...
    source_job_id: Optional[UUID] = Field(
        default=None, foreign_key="return_jobs.id", ondelete="SET NULL"
    )
...
class PickingInstruction(SQLModel, table=True):
    ...
    instruction_no: str = Field(
        max_length=50, unique=True, index=True
    )  # PICK-YYMMDD-####
    status: str = Field(
        default="PENDING", max_length=20
    )  # PickingInstructionStatusEnum
    ...
    worker_note: Optional[str] = Field(
        default=None, sa_column=Column(Text)
    )  # LLM 작업자 지시문
    ...
    cushion_name: Optional[str] = Field(
        default=None, max_length=100
    )  # 확정 완충재 (worker 포장 가이드용)
...
class PickingInstructionItem(SQLModel, table=True):
    ...
    id: UUID = Field(default_factory=uuid4, primary_key=True)
    instruction_id: UUID = Field(
        foreign_key="picking_instructions.id", ondelete="CASCADE", index=True
    )
    order_item_id: Optional[UUID] = Field(
        default=None, foreign_key="order_items.id", ondelete="SET NULL"
    )
    book_id: UUID = Field(foreign_key="books.id", ondelete="RESTRICT")
    used_item_id: Optional[UUID] = Field(
        default=None, foreign_key="inventory_used_items.id", ondelete="SET NULL"
...
class OrderProposal(SQLModel, table=True):
    ...
    trigger_type: str = Field(
        default="INSPECTION_REJECT", max_length=30
    )  # OrderProposalTriggerEnum
    ...
    current_stock: int = Field(
        default=0
    )  # 가용 재고 = 신품 virtual_stock + 중고 IN_STOCK 합산
    sales_velocity_30d: int = Field(
        default=0
    )  # 최근 30일 출고량 (InventoryLog OUTBOUND 집계)
    rejected_quantity: int = Field(default=0)  # 이번 반려로 소실된 매입 예정 수량
    baseline_quantity: int = Field(
        default=0
    )  # 결정론적 안전재고 산식 기준 수량 (Validator 클램프 앵커)
    ...
    ai_source: str = Field(
        default="LLM_GPT4O_MINI", max_length=30
    )  # LLM_GPT4O_MINI | FALLBACK_RULE
...
class Board(SQLModel, table=True):
    ...
    job_id: Optional[UUID] = Field(
        default=None, foreign_key="return_jobs.id", ondelete="CASCADE"
    )
...
class WeeklyInsight(SQLModel, table=True):
    ...
    top_defective_publishers: Optional[Dict[str, Any]] = Field(
        default=None, sa_column=Column(JSONB)
    )
    location_hotspots: Optional[Dict[str, Any]] = Field(
        default=None, sa_column=Column(JSONB)
    )
    logistics_hotspots: Optional[Dict[str, Any]] = Field(
        default=None, sa_column=Column(JSONB)
    )
...
class AdminAuditLog(SQLModel, table=True):
    ...
    defect_coordinates: Optional[List[Dict[str, Any]]] = Field(
        default=None, sa_column=Column(JSONB)
    )
...
class LabelPrintJob(SQLModel, table=True):
    ...
    zpl: str
    status: str = Field(
        default="PENDING", max_length=12, index=True
