"""Admin HITL 업무 규칙 - 결재 오버라이드·회수·재검수 큐잉·점수 미리보기·보조 브리핑.

본문은 router.py에서 무수정 이동했다 (2026-09-01 계층 정리). 요청·응답 스키마는
schemas.py, 권한(RoleChecker)·배선은 router.py가 담당한다.

`_is_hitl_required`(참조 0건)는 이동하지 않고 제거했다 - `_is_human_owned`가
대체 구현이다 (경위는 그 docstring에 있다). 원본: archive/2026-09-01_service_layer_refactor/.
"""
...
def get_pending_hitl_tasks(session: Session) -> List[HitlTaskResponse]:
    """
...
def trigger_ai_reinspection(session: Session, job_id: str):
    """
...
def get_completed_hitl_tasks(session: Session) -> List[HitlTaskResponse]:
    """
