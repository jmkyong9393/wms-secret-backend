"""대시보드 SQL 집계 로직 - KPI·검수경로·차트·로그·AI 품질.

본문은 router.py에서 무수정 이동했다 (2026-09-01 계층 정리).
주간 인사이트는 종전대로 weekly_insight_service가 담당한다.
"""
...
def get_kpi(session: Session) -> Dict[str, Any]:
    """
...
def get_inspection_breakdown(session: Session) -> Dict[str, Any]:
    """
...
def get_ai_quality_stats(session: Session) -> Dict[str, int]:
    """
