"""업로드 보안 검증·S3 오케스트레이션 - 화이트리스트, presign 발급, 격리 검사·승격.

본문은 router.py에서 무수정 이동했다 (2026-09-01 계층 정리). HTTP 응답 조작
(Signed Cookie 세팅)만 라우터에 남는다.
"""
...
def authorize_cloudfront_upload(file_name: str, category: str) -> Dict[str, str]:
    """파일명 검증 후 CloudFront Signed Cookie 값을 생성한다 (쿠키 세팅은 라우터 몫)."""
    ...
    try:
        return generate_signed_cookies(resource_url=resource_url, expire_minutes=15)
...
def get_presigned_url(file_name: str, file_type: str) -> dict:
    """S3 다이렉트 업로드용 Pre-signed URL 발급 (자격증명 없으면 Mock URL)."""
...
def presign_attachment_upload(
    current_user: User, file_name: str, file_type: str
) -> dict:
...
def verify_attachment_upload(current_user: User, object_key: str) -> dict:
...
def issue_attachment_download_urls(object_keys: List[str]) -> Dict[str, Dict[str, str]]:
