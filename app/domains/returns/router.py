...
def create_inspection(
    request: InspectionRequest, return_service: ReturnService = Depends()
):
    """
...
def list_inspections(
    ...
    status: Optional[str] = Query(
        None,
        description="APPROVED / REJECTED / HITL_REQUIRED / PENDING / PROCESSING / FAILED",
    ),
    scope: Optional[str] = Query(
        None, description="mine이면 로그인 사용자 본인 검수만"
    ),
    worker_id: Optional[str] = Query(
        None, description="특정 담당자 사번으로 필터 (ADMIN/MASTER 전용)"
    ),
    ...
    rows = db.exec(
        stmt.order_by(ReturnJob.created_at.desc()).limit(limit).offset(offset)
    ).all()
    ...
    for job in rows:
        ...
        item = (
            db.exec(
                select(InventoryUsedItem).where(InventoryUsedItem.lpn_barcode == lpn)
            ).first()
            if lpn
            else None
        )
        ...
        items.append(
            {
                ...
                "status": job.status,
                "inspection_source": getattr(item, "inspection_source", None)
                if item
                else None,
                "inspected_by": getattr(item, "inspected_by", None) if item else None,
                # 재고 상세와 같은 표기를 쓰도록 라벨 생성은 한 곳에서만 한다.
                "inspector_label": resolve_inspector(item, job)["label"]
                if item
                else None,
    ...
    if include_new:
        ...
        log_rows = db.exec(
            log_stmt.order_by(InventoryLog.created_at.desc()).limit(limit)
        ).all()
        new_books = {}
        if log_rows:
            ...
            items.append(
                {
                    ...
                    "updated_at": lg.updated_at.strftime("%Y-%m-%d %H:%M:%S")
                    if lg.updated_at
                    else None,
                }
            )
    ...
    for it in items:
        it["worker_label"] = format_worker_label(
            it.get("inbound_worker_id"),
            name_by_emp.get(it.get("inbound_worker_id") or ""),
        )
