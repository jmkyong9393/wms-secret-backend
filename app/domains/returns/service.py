...
class ReturnService:
    ...
    def trigger_inspection(
        self, book_id: UUID, location_id: UUID, image_urls: List[str]
    ) -> UUID:
        """
        ...
        new_job = ReturnJob(
            book_id=book_id, image_urls=image_urls, status=JobStatusEnum.PENDING.value
        )
        self.db.add(new_job)
        ...
        try:
            ...
            threading.Thread(
                target=process_inspection, args=(str(new_job.id),), daemon=True
            ).start()
