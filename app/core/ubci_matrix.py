...
[이 파일이 존재하는 이유]
감점 수치는 원래 policy_agent 함수 안에 리터럴로 흩어져 있었다. 그 숫자들은 사람이 `docs/ai_knowledge_base/WMS_표준_운영_정책서.md` 제4조⑤에서 옮겨 적은 값인데, 옮겨 적은 뒤로는 두 곳이 각자 갱신되며 조용히 벌어졌다(오염·황변에서 실제로 어긋났다).
...
[RAG 인용과의 차이 - 중요]
이 조항 라벨은 결정론적이다. 벡터 검색 결과가 아니라 코드에 고정된 매핑이므로 RAG 서버가 죽어도 근거 표기가 사라지지 않고, 실행할 때마다 같은 조항이 나온다.
RAG(`rag_service.cite_deduction_basis`)는 조문 전문 발췌를 덧붙이는 보조 역할이다.
...
# 황변은 페이지 전면에 나타나 면적비가 항상 ~100%가 된다. 면적으로 재면 세월 먹은 정상적인 헌책이 매번 최고 감점을 맞으므로, VLM이 보고한 강도(level 1~3)를 축으로 쓴다. L1~L2는 등급에 영향을 주지 않는 수준으로 관대하게 둔다(중고책의 자연 노화).
