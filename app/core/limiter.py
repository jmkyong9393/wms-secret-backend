...
Next.js의 rewrites 프록시 뒤에 있어서, 컨테이너가 보는 peer IP가 접속자와 무관하게 항상 도커 게이트웨이(172.20.0.1) 하나로 수렴한다. 그 결과 "5회/분" 제한이 접속자별이 아니라 서비스 전체 합산으로 동작해, 한 사람이 오타를 몇 번 내면 나머지 전원이 로그인하지 못했다 (cloudflared 터널 시연 환경에서 실측 확인).
여기서는 요청이 신뢰 대역(도커 브리지/루프백)에서 들어온 경우에 한해 X-Forwarded-For의 원 클라이언트 IP를 키로 채택한다. 신뢰 대역 밖에서 온 XFF는 헤더 위조로 리밋을 우회하는 수단이 되므로 무시하고 peer IP를 그대로 쓴다.
"""
