...
종전 검증은 `min_length=4` 하나가 전부였다. 초기 암호 "1234"를 그대로 새 비밀번호로 재설정해도 통과했다는 뜻이다.

규칙 (해설서의 "안전한 비밀번호 작성규칙"):
- 영문 대/소문자, 숫자, 특수문자 중 2종류 조합이면 10자리 이상, 3종류 이상 조합이면 8자리 이상 연속되거나 동일한 문자/숫자 반복 금지 (예: 1234, aaaa, abcd).
...
# 화면 안내 문구. 서버 규칙이 바뀌면 이 목록도 같은 파일에서 함께 바뀌므로 설명과 검증이 어긋날 수 없다 (프론트는 GET /auth/password-policy로 이 값을 그대로 받아 쓴다).
POLICY_DESCRIPTIONS = [
